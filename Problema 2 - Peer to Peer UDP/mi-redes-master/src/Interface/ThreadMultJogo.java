 ...
 * @author allen
 */
public class ThreadMultJogo implements Runnable {
    ...
    public ThreadMultJogo(Player p, Game3 jogo) {
        this.p = p;
        this.jogo = jogo;
    }
    ...
    public void run() {
        while (true) {
            ...
            jogo.getP3().setText(p.getGame().getUsername(p.getIDS().get(1)));
            ...
            try {
             ...
             try {
                jogo.getPontuacao3().setText("" + p.getGame().getPlayerScore(p.getIDS().get(1)));
            } catch (Exception x) {
                jogo.getPontuacao3().setText("" + 0);
            }
            if (p.isMyTurn()) {
